    ...
    var body: some View {
        NavigationStack {
            VStack(spacing: 0) {
                ...
                HStack { TextField("What’s happening?", text: $model.draft, axis: .vertical).textFieldStyle(.roundedBorder).accessibilityIdentifier("coachInput"); Button("Send") { Task { await send() } }.tint(Color(red: 0, green: 0.25, blue: 0.65)).disabled(model.draft.trimmingCharacters(in: .whitespaces).isEmpty || model.isLoading) }.padding()
